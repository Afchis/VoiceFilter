...
class SpeechEmbedder(nn.Module):
    def __init__(self):
        ...
        self.loss_w = nn.Parameter(torch.tensor([1.]))
        self.loss_b = nn.Parameter(torch.tensor([0.]))
        self.loss_w.requires_grad = False
        self.loss_b.requires_grad = False
    ...
    def _softmax_loss(self, pos_sim, neg_sim):
        ...
        return loss.sum(), pos_sim.mean().item(), neg_sim.mean().item()
    ...
    def forward(self, mel):
        ...
        dvec = self.proj(dvec) # (b, T, emb_dim)
        dvec = dvec.div(dvec.norm(p=2, dim=1, keepdim=True)).mean(dim=1)
        if self.train:
