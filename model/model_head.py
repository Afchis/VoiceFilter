...
class VoiceFilter(nn.Module):
    def __init__(self, RE_weights="./weights/voice_encoder/embedder.pt"):
        ...
        self.refer_encoder.load_state_dict(torch.load(RE_weights, map_location=torch.device('cpu')))
        ...
        self.lstm = nn.LSTM(input_size=(8*512)+256, hidden_size=400)
        self.fc = LinReluBlock(in_ch=400, out_ch=512)
    ...
    def forward(self, refer_spec, noicy_spec):
        ...
        d_vec = d_vec.expand(d_vec.size(0), out.size(1), d_vec.size(1))
